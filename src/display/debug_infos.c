...
/*   Updated: 2025/09/06 09:56:34 by lvan-bre         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	put_debug_info_2(t_data *data)
{
	...
	mlx_string_put(data->win.mlx, data->win.window, 500, 100, 0000000000, txt);
	ft_str_reset(&txt);
	ft_smprintf(&txt, "PlaneY : %f", data->player->plane[Y]);
	mlx_string_put(data->win.mlx, data->win.window, 500, 120, 0000000000, txt);
	ft_str_reset(&txt);
}
